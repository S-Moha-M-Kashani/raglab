...
# Re-exported: the option tuples live in options.py — the lab's closed
# vocabularies — because dependencies.py needs a dozen of them and must not
...
# `config.CHUNKERS` etc.
from .options import (CHUNKERS, CHAR_SIZED_CHUNKERS, OVERLAP_CHUNKERS,  # noqa: F401
...
# Re-exported: DEPENDENCIES lives in dependencies.py, which now reads the
# option tuples from options.py rather than from here, so this import carries
