...
def _knn_edges(vectors: np.ndarray, k: int) -> dict[tuple[int, int], float]:
    ...
    # Guards on < 1e-12, not == 0 like in_memory_vector_store.py/embedding_backends.py: a tiny-but-nonzero norm is left unscaled rather than blown up.
...
def summarize(chunks: list[Chunk], vectors: np.ndarray, members: list[int],
    ...
    if summarizer == 'mmr':
        from raglab.rag_components.retrieval.retrieve_fuse_rerank_grade import mmr
        block = vectors[members]
        centre = block.mean(axis=0)
