...
not carry; `claude`/`codex` run a CLI subprocess instead (`cli_subprocess_chat.py`) but arrive
...
def make_chat_model(settings, model: str = '') -> BaseChatModel:
    ...
    if settings.provider in clichat.CLIS:
        ...
        # machine's own CLI login. See cli_subprocess_chat.py for why each argv flag matters.
