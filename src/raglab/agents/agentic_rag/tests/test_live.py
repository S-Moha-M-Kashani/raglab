...
the guard is invocation-shaped, the same as test_skills_live.py's: the
...
the real loop against a real model with the real `.env`.
    ...
    uv run pytest src/raglab/agents/agentic_rag/test_live.py -v -s

test_agentic_rag.py holds the offline coverage (caps, stop reasons,
...
from raglab.configuration.env_settings import load_lab_settings
