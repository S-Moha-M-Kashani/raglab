...
a Markdown body. `raglab/skills_corpus_loader.py` is the loader, and the widget gains two
...
`test_skills_live.py`; it skips under any broad sweep and runs only
when named: `uv run pytest test_skills_live.py -v -s`.
"""
import os
...
def test_the_index_serves_every_skill_with_its_description():
    ...
    for name, description in index.items():
        ...
        assert (ROOT / 'fixtures' / 'skills' / name / 'SKILL.md').is_file()
...
def test_the_index_covers_every_skill_folder_in_the_repository():
    ...
    folders = {p.parent.name for p in (ROOT / 'fixtures' / 'skills').glob('*/SKILL.md')}
