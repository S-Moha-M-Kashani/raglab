"""The served frontend: the files the panel makes public, and the markup,
stylesheet and script they carry.

The one file in this suite that asserts on served frontend. Its convention
table is the bulk of it — one row per rule, each naming the file it is a claim
about and the reason a failure should print — and the tests around it are the
relationships a substring cannot hold: which knobs the layout reaches, which
theme outranks which, and that the three lists of knobs (the dataclasses, the
export codec, the controls) still name the same knobs.
"""
import shutil
import subprocess
import re
