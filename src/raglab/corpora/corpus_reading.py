...
def document_text(document: dict) -> str:
    """One document as plain text, part by part, for embedding. A part whose
    `labels` carry a `role` is prefixed with that role's own value — the raw
    label, not a language-specific translation of it, since not every corpus
    declares one."""
    lines = []
